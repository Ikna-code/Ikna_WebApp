...
export async function syncOrderState(input: SyncOrderInput) {
  const order = await db.order.findFirst({
    where: {
      OR: [
        ...
        input.razorpayOrderId ? { razorpayOrderId: input.razorpayOrderId } : undefined,
        input.shiprocketOrderId ? { shiprocketOrderId: input.shiprocketOrderId } : undefined,
        input.shipmentId ? { shipmentId: input.shipmentId } : undefined,
  ...
  const updatedOrder = await db.$transaction(async (tx) => {
    ...
    const awbCode = coerceString(input.shipment?.awbCode);
